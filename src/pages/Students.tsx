    ...
    <DashboardLayout>
      <div className="flex flex-col h-full space-y-2">
        ...
        <Card className="flex-1 flex flex-col min-h-0 overflow-hidden">
          <CardHeader className="shrink-0 py-2 px-3 sm:px-4">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm sm:text-base">All Learners ({filteredLearners.length})</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="flex-1 overflow-hidden p-0">
            {loading ? (
              ...
              <div className="text-center py-6">
                <p className="text-muted-foreground mb-3 text-sm">No learners found</p>
                ...
                <table className="w-full text-xs">
                  <thead className="border-b border-border bg-muted/50 sticky top-0 z-10">
                    <tr className="text-left font-medium text-muted-foreground">
                      <th className="p-2 whitespace-nowrap w-10">Photo</th>
                      <th className="p-2 whitespace-nowrap">Adm No.</th>
                      <th className="p-2 whitespace-nowrap">Name</th>
                      <th className="p-2 whitespace-nowrap hidden lg:table-cell">Gender</th>
                      <th className="p-2 whitespace-nowrap hidden md:table-cell">Grade</th>
                      <th className="p-2 whitespace-nowrap hidden md:table-cell">Stream</th>
                      <th className="p-2 whitespace-nowrap hidden xl:table-cell">Status</th>
                      <th className="p-2 whitespace-nowrap hidden xl:table-cell">Boarding</th>
                      <th className="p-2 whitespace-nowrap text-right w-10">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {filteredLearners.map((learner) => (
                      ...
                      >
                        ...
                        <td className="p-2">
                          <span className="font-medium text-primary whitespace-nowrap text-xs">
                        ...
                        <td className="p-2 hidden lg:table-cell">
                          <span className="text-muted-foreground capitalize text-xs">{learner.gender || 'N/A'}</span>
                        </td>
                        <td className="p-2 hidden md:table-cell">
                          <Badge variant="outline" className="text-[10px] px-1.5 py-0">{learner.current_grade?.name || 'N/A'}</Badge>
                        </td>
                        <td className="p-2 hidden md:table-cell">
                          <Badge variant="secondary" className="text-[10px] px-1.5 py-0">{learner.current_stream?.name || 'N/A'}</Badge>
                        </td>
                        <td className="p-2 hidden xl:table-cell">
                          <Badge variant={learner.status === 'active' ? 'default' : 'secondary'} className="text-[10px] px-1.5 py-0 capitalize">
                            {learner.status || 'active'}
                          </Badge>
                        </td>
                        <td className="p-2 hidden xl:table-cell">
                          <span className="text-muted-foreground capitalize text-xs">{learner.boarding_status || 'day'}</span>
                        </td>
