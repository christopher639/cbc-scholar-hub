-- Update the generate_learner_invoice function to include previous unpaid balances
...
BEGIN
  ...
  -- Calculate previous unpaid balance from all previous invoices
  SELECT COALESCE(SUM(balance_due), 0) INTO v_previous_balance
  FROM student_invoices
  WHERE learner_id = p_learner_id
    AND status NOT IN ('paid', 'cancelled')
    AND (academic_year < p_academic_year OR (academic_year = p_academic_year AND term < p_term::term));
  ...
  -- Apply staff discount if eligible (only on current term fees, not previous balance)
  ...
  -- Add previous balance to total amount
  v_total_amount := v_total_amount + v_previous_balance;
  ...
  -- Add previous balance as a line item if there is any
